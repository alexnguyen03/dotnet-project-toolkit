...
export class ProfileService implements IProfileService {
    ...
    async create(projectInfo: ProjectInfo, data: ProfileWizardData, overwrite: boolean = false): Promise<string | null> {
        try {
            ...
            const targetFramework = await this.projectAnalyzer.detectTargetFramework(projectInfo.csprojPath);
            ...
            const pubxmlPath = await this.profileRepository.save(projectInfo, data.profileName, content, overwrite);
            if (!pubxmlPath) {
                return null;
            }
    ...
    parse(pubxmlPath: string): PublishProfileInfo | null {
        try {
            ...
            const environment = this.environmentDetector.detectFromXml(envName) 
