...
export interface IProfileService {
    ...
    update(profilePath: string, data: ProfileWizardData): Promise<boolean>;
...
export class ProfileService implements IProfileService {
    ...
    async update(profilePath: string, data: ProfileWizardData): Promise<boolean> {
        try {
            // Extract project directory from profile path
            const projectDir = profilePath.replace(/[\\\/]Properties[\\\/]PublishProfiles[\\\/].*$/, '');
            const csprojFiles = fs.readdirSync(projectDir).filter(f => f.endsWith('.csproj'));
            const csprojPath = csprojFiles.length > 0 ? path.join(projectDir, csprojFiles[0]) : '';

            // Detect .NET version
            const targetFramework = await this.detectTargetFramework(csprojPath);

            // Generate XML content
            const content = this.generatePubxmlContent(data, targetFramework);

            // Write directly to existing file (no overwrite prompt)
            fs.writeFileSync(profilePath, content, 'utf-8');

            this.log(`Updated profile: ${profilePath}`);
            return true;
        } catch (error) {
