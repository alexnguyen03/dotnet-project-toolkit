...
export class EditProfileCommand extends BaseCommand {
	...
	async execute(item?: unknown): Promise<void> {
		...
		const projectInfo = {
			...
			projectDir: profileInfo.path.replace(
	...
	private async collectProfileData(
		existing: PublishProfileInfo
	): Promise<ProfileWizardData | undefined> {
		...
		// Step 2: Environment
		const currentEnv =
			existing.environment === DeployEnvironment.Unknown
			...
			[
				{
					...
					description: currentEnv === DeployEnvironment.Staging ? '(current)' : '',
				},
				{
					label: 'Production',
					value: DeployEnvironment.Production,
					description: currentEnv === DeployEnvironment.Production ? '(current)' : '',
				},
				{
					label: 'Development',
					value: DeployEnvironment.Development,
					description: currentEnv === DeployEnvironment.Development ? '(current)' : '',
				},
			],
