...
export class DeployProfileCommand implements ICommand {
	...
	async execute(item: PublishTreeItem): Promise<void> {
		...
		if (!projectPath && profile.path) {
			...
			const projectDir = pubxmlPath.replace(
				/[\\\/]Properties[\\\/]PublishProfiles[\\\/][^\\\/]+\.pubxml$/i,
				''
			);
			...
			try {
				if (fs.existsSync(projectDir)) {
					...
					if (csprojFile) {
						...
						this.outputChannel.appendLine(
							`[Deploy] Derived project path from pubxml: ${projectPath}`
						);
					}
				}
			} catch (error) {
		...
		try {
			...
			await vscode.window.withProgress(
				...
				async (progress) => {
					this.outputChannel.appendLine(
						`[Deploy] Starting deployment for ${profile.name} (${environment})`
					);
					this.outputChannel.appendLine(`[Deploy] Project: ${projectName}`);
					this.outputChannel.appendLine(`[Deploy] Profile Path: ${profile.path}`);
					this.outputChannel.appendLine(
						`[Deploy] Using Project Path: ${projectPath || 'NOT FOUND'}`
					);

					if (!projectPath) {
						throw new Error(
							'Project path not found. Cannot deploy without .csproj path.'
						);
					}
			...
			await this.historyManager.updateDeployment(
				historyId,
				{
					status: 'success',
			...
			if (openBrowser && profile.siteUrl) {
				...
				if (openBrowser && !profile.siteUrl) {
					this.outputChannel.appendLine(
						`[Browser] No site URL configured for this profile`
					);
				}
