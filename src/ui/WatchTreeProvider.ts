	...
	async getChildren(element?: WatchTreeItem): Promise<WatchTreeItem[]> {
			...
			if (element.id === 'watch-groups') {
				...
				return groups.map(
					(g) =>
						new WatchGroupItem(
							g,
							this.watchService.isGroupRunning(g, structure.projects)
						)
				);
			}
		}
		...
		if (element instanceof ProjectContainerItem) {
			...
			return sortedProjects.map(
				(p) => new ProjectItem(p, this.watchService.isProjectRunning(p.csprojPath))
			);
		}
...
export class RunningWatchItem extends vscode.TreeItem {
	constructor(
		...
		public readonly csprojPath: string
	) {
