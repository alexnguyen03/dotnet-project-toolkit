...
export class ConnectionTester {
	...
	private static async testMSDeployConnection(
		profile: PublishProfileInfo,
		outputChannel?: vscode.OutputChannel
	): Promise<{ success: boolean; message: string }> {
				...
				const req = requestLib.request(
					...
					(res) => {
						outputChannel?.appendLine(
							`[TestConnection] Status Code: ${res.statusCode}`
						);
	...
	private static async testFileSystemConnection(
		profile: PublishProfileInfo,
		outputChannel?: vscode.OutputChannel
	): Promise<{ success: boolean; message: string }> {
		...
		if (fs.existsSync(targetPath)) {
			try {
				...
				return {
					success: true,
					message: `Directory exists and is writable: ${targetPath}`,
				};
			} catch (err: any) {
				return {
					success: false,
					message: `Directory exists but not writable: ${err.message}`,
				};
			}
		} else {
			...
			try {
				...
				if (fs.existsSync(parent)) {
					return {
						success: true,
						message: `Target directory does not exist but parent is reachable. It will be created on deploy.`,
					};
				}
			} catch (e) {
				/* ignore */
