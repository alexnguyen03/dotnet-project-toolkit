...
export class ProjectScanner {
	...
	private detectServerClientStructure(projects: ProjectInfo[]): boolean {
		const hasServer = projects.some(
			(p) => p.projectDir.includes('Server') || p.projectDir.includes('server')
		);
		const hasClient = projects.some(
			(p) => p.projectDir.includes('Client') || p.projectDir.includes('client')
		);
